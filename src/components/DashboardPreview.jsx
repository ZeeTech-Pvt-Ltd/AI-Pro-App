...
const WATCHLIST = [
  { name: 'Bitcoin (BTC)', change: '+1.4%', up: true, dot: '#06B6D4' },
  { name: 'Ethereum (ETH)', change: '+0.6%', up: true, dot: '#7C3AED' },
  { name: 'Solana (SOL)', change: '-0.4%', up: false, dot: '#4338CA' },
  { name: 'Cardano (ADA)', change: '+0.9%', up: true, dot: '#4B4E6D' },
];

const ALLOCATION = [
  { label: 'Bitcoin', value: '42%', pct: 42, color: '#06B6D4' },
  { label: 'Ethereum', value: '28%', pct: 28, color: '#7C3AED' },
  { label: 'Altcoins', value: '30%', pct: 30, color: '#4338CA' },
];
...
const ACTIVITY = [
  { icon: Sparkles, text: 'Scanned 120+ assets this morning' },
  { icon: TrendingUp, text: 'Saved a chart view for later' },
  { icon: Bell, text: 'Set an alert on Bitcoin' },
];
      ...
      <div className="shell">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 id="preview-heading" className="mt-5 text-[clamp(1.9rem,4vw,2.75rem)] font-extrabold leading-tight tracking-tight text-primary">
            Everything you research, in one clear view
          </h2>
        ...
        <div
          ...
          role="img"
          aria-label="Illustrative preview of the AI Pro App console showing market scanning, watchlist, charting and activity panels"
        >
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-5">
            <div className="flex items-center gap-3">
              ...
              <p className="ml-1 text-sm font-semibold text-white/90">My research workspace</p>
            </div>
          ...
          <div className="relative mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-hidden="true">
              ...
              <ul className="mt-3 space-y-2.5">
                {WATCHLIST.map((row) => (
                  <li key={row.name} className="flex items-center justify-between text-sm">
                    ...
                    <span className={`text-xs font-semibold ${row.up ? 'text-accent-dark' : 'text-red-500'}`}>
                      {row.up ? '▲' : '▼'} {row.change}
                    </span>
                  </li>
                ))}
            ...
            {/* Market coverage */}
            <div className="rounded-2xl bg-white/95 p-5 shadow-card xl:col-span-2">
              <div className="flex items-center gap-2 text-muted">
              ...
              <div className="mt-3 flex items-baseline gap-2">
                <p className="font-display text-2xl font-extrabold text-ink">120+</p>
                <p className="text-[11px] text-muted">Assets tracked · illustrative</p>
              </div>
            ...
            <div className="rounded-2xl bg-white/95 p-5 shadow-card xl:col-span-2">
              ...
              <ul className="mt-3 space-y-3">
                {ACTIVITY.map(({ icon: Icon, text }) => (
                  <li key={text} className="flex items-center gap-3 text-sm text-steel">
                    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-accent/10 text-accent-dark">
          ...
          <p className="relative mt-5 text-center text-[11px] text-white/50">
            Preview shown is an illustrative mock-up for demonstration — not real market data or
            financial advice.
          </p>
        </div>
      </div>
